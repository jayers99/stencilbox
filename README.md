# Stencilbox

![Stencilbox banner](shared/StencilBoxBanner.png)

Templates, prompts, and workflows for AI-assisted creation - code, images, and writing.

## Structure

```
stencilbox/
├── code/           # Software development templates
├── images/         # Image generation prompts & workflows
├── writing/        # Fiction, nonfiction, copywriting
├── learning/       # Programming skill development
├── projects/       # Active project-specific stencils
└── shared/         # Cross-domain personas & workflows
```

## Quick Start

### Code Projects

```
Read code/discovery/PROCESS.md and help me brainstorm a new project.
Read code/bootstrap/PROCESS.md and create a Python CLI project.
Read code/Coding_Human_AI_Team_Agreement.md and help me get started.
```

See [code/README.md](code/README.md) for full development workflow.

### Image Generation

| Tool | Location |
|------|----------|
| DALL-E / ChatGPT | `images/prompts/dalle/` |
| Midjourney | `images/prompts/midjourney/` |
| Gemini / Nano Banana | `images/prompts/gemini/` |
| Style references | `images/styles/` |
| Multi-step workflows | `images/workflows/` |

### Writing

| Type | Location |
|------|----------|
| Fiction | `writing/fiction/` |
| Nonfiction | `writing/nonfiction/` |
| Prompts | `writing/prompts/` |

### Learning

Track programming skill development:

| Folder | Purpose |
|--------|---------|
| `problem-solving/` | Algorithms, debugging, thinking frameworks |
| `fundamentals/` | CS concepts, design patterns |
| `journal/` | Progress tracking, reflections |

## Active Projects

| Project | Description |
|---------|-------------|
| [bowerbird](projects/bowerbird/) | Photo collection to illustration generator |
| [pebble](projects/pebble/) | Scrum standup helper |

## Shared Resources

- **Personas** (`shared/personas/`) - AI personalities for different tasks
- **Workflows** (`shared/workflows/`) - Cross-domain processes

## Philosophy

Stencils, not scaffolds. These are patterns to trace, not structures to fill. Use them as starting points, adapt freely, discard what doesn't fit.
